...
class Vehicle:
    def __init__(self, screen: pg.Surface) -> None:
        ...
        self.original_image: pg.Surface = pg.image.load(f"{IMAGES_PATH}/tractor_0.png")
        ...
        self.hitch: Hitch = Hitch(0, 14, self.rect.center)
...
class Trailer:
    def __init__(self, screen: pg.Surface, vehicle: Vehicle) -> None:
        self.screen: pg.Surface = screen
        self.vehicle: Vehicle = vehicle
        self.original_image: pg.Surface = pg.image.load(f"{IMAGES_PATH}/tool_0.png")
        self.image: pg.Surface = self.original_image
        self.rect: pg.Rect = self.image.get_rect()
        self.x: float = 200.0
        self.y: float = 240.0
        self.update_rect()
        self.rotation: float = 0.0
        self.hitch: Hitch = Hitch(0, -5, self.rect.center)
    ...
    def direction_to_vehicle(self) -> float:
        """
        Degrees
        """
    ...
    def draw(self) -> None:
        self.rotation %= 360
        self.rotation = self.direction_to_vehicle()
...
class Test:
    def __init__(self) -> None:
        ...
        self.vehicle: Vehicle = Vehicle(self.screen)
        self.trailer: Trailer = Trailer(self.screen, self.vehicle)
    ...
    def main(self) -> None:
        while 1:
            ...
            if keys[pg.K_a]: self.vehicle.rotation += 1 * self.vehicle.speed
            if keys[pg.K_d]: self.vehicle.rotation -= 1 * self.vehicle.speed
