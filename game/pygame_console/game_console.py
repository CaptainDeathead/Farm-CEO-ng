...
class CommandLineProcessor(cmd.Cmd):
	...
	def do_set_paddock_state(self, params):
		self.app.win.farm_ceo.paddock_manager.set_paddock_state(1, 5)

	def do_sps(self, params):
		self.do_set_paddock_state(params)
...
class Console(pygame.Surface):
	...
	def __init__(self, app, width, config={}):
		...
		self.console_header = Header(self, (self._width - self.padding.left - self.padding.right), config.get('header')) if config.get('header', None) else None

		# Initiate input text object
		self.console_input = TextInput(self, (self._width - self.padding.left - self.padding.right), config.get('input')) if config.get('input', None) else None

		# Initiate output text object
		self.console_output = TextOutput(self, (self._width - self.padding.left - self.padding.right), config.get('output')) if config.get('output', None) else None

		# Initiate footer object
		self.console_footer = Header(self, self._width - self.padding.left - self.padding.right, config.get('footer')) if config.get('footer', None) else None		
		...
		self.dim = (self._width, self.padding.up 
